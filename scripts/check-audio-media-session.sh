...
require_contains() {
  ...
  if ! rg -q -F "$expected_text" "$file_path"; then
...
player_count="$(rg -o '<audio controls class="player"' "$output_dir" | wc -l | tr -d ' ')"
title_count="$(rg -o 'data-title="[^"]+"' "$output_dir" | wc -l | tr -d ' ')"
artist_count="$(rg -o 'data-artist="[^"]+"' "$output_dir" | wc -l | tr -d ' ')"
